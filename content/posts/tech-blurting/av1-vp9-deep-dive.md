...
이번 글은 **MPEG 진영의 라이센스 지옥을 피하려고 만든 무료 코덱들**의 진화 — VP8 → VP9 → AV1, 그리고 그 부산물인 **AVIF(이미지 포맷)**이 어떻게 JPEG을 대체하고 있는지 — 정리한 노트다. 레벨 3 (코덱) 시리즈 마지막.
...
레벨 3 (코덱) 시리즈는 여기까지. 다음 레벨에서는 **트랜스코딩 인프라** — FFmpeg, NVENC, ABR Ladder, Per-Title Encoding 등 — 운영의 세계로 들어간다.
